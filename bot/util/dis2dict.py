...
def Message(msg:discord.Message):
    return {
        'TTS':msg.tts,
        'TYPE':str(msg.type),
...
def Embed(emb:discord.Embed):
    return emb.to_dict()


def CallMessage(cll:discord.CallMessage):
    return {
        'ENDED':str(cll.ended_timestamp),
...
def GroupChannel(grp:discord.GroupChannel):
    return {
        'PEOPLE':Users(grp.recipients),
        'ID':grp.id,
...
def abcUser(abc:discord.abc.User):
    return {
        'NAME':abc.name,
        'ID':abc.id,
...
def abcGuildChannel(abc:discord.abc.GuildChannel):
    return {
        'NAME':abc.name,
        'GUILD': None,
        'ID':abc.id,
        'DISCRIMINATOR':abc.discriminator,                                                   'AVATAR':abc.avatar,
        'BOT':abc.bot,
        'DISPLAY NAME':abc.display_name,
        'AVATAR URL':str(abc.avatar),
        'MENTION':abc.mention
