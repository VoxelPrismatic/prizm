...
async def hangman(ctx):
    ...
    def check(m):
        try:
            return m.author.id in list(games) and m.channel.id == games[m.author.id][1]
        except:
            return False
    games[ctx.author.id] = [msg,msg.channel.id]
    y = " "
    while ''.join(st2.split()) != st and y != "STOP" and guess < 6:
        try:
            v = await ctx.bot.wait_for('message',timeout = 60, check=check)
        except:
            count += 1
            if count >= len(list(games)):
                games = {}
                count = 0
        y = v.content.upper()
        st2 = st2.replace('\_','_').split()
        wrong = True
