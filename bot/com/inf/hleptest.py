...
@commands.command(aliases=["helptest"])
async def hleptest(ctx):
    pages.PageThis()
    lit = ["""#] INFO
] ";]hlep"
...
"""]
    await pages.PageThis.page(ctx, lit, "COMMANDS LIST", """```md
...
def setup(bot):
    print('+COM')
    bot.add_command(hleptest)
    print('GOOD')

def teardown(bot):
    print('-COM')
    bot.remove_command('hleptest')
    print('GOOD')
