...
async def mix(ctx,*, arg):
    ...
    elif arg.split()[0] in ['slot2','s2','2','img2']:
        if open('mix/status').read() != 'DONE':
            return await ctx.send('```diff\n-] I AM GENERATING SOMETHING, CHECK BACK LATER\n=] Or use \';]mix view\' to see the latest iter```')
        try:
            if ctx.message.attachments[0].size > 1024*1024*4:
    ...
    elif arg.split()[0] in ['run','start','slots']:
        if open('mix/status').read() != 'DONE': return await ctx.send('```diff\n-] I AM GENERATING SOMETHING, CHECK BACK LATER\n=] Or use \';]mix view\' to see the latest iter```')
        mc = ctx.message.content
        settings = {'iter':60,
                    'ping': True}
        mc += ' '
        if '-i=' in mc: settings['iter'] = int(mc[mc.find('-i=')+3:mc.find(' ',mc.find('-i='))])
        elif '-iter=' in mc: settings['iter'] = int(mc[mc.find('-iter=')+6:mc.find(' ',mc.find('-iter='))])
        ...
        while open('mix/status').read() != 'DONE':
            ...
            while loc == open('mix/loc').read() and open('mix/status').read() != 'DONE':
                try: m = await ctx.bot.wait_for('message',check=chek,timeout=10.0)
                    ...
                    if m.content.lower() in [';]mix kill',';]mix stop',';]mix clear',';]mix end',';]mix break',';]mix complete',';]mix empty',';]smix send']:
                        open('mix/status','w').write('DONE')
                        os.kill(proc.pid,signal.SIGKILL)

        return await ctx.send((f'<@{ctx.author.id}> ' if settings['ping'] else '')+f'```md\n#] GENERATED!```',file=discord.File('mix/now.png'))

    elif arg.split()[0] in ['see','view','where']:
        loc = open("mix/loc").read(); ttl = open("mix/iter").read(); stt = open('mix/status').read()
        try: content = f'`[{int(loc)/int(ttl)*100:.2f}% - {loc}/{ttl}] {stt}`'
        except: content=f'`[{loc}] {stt}`'
        await ctx.send(content,file=discord.File(fp=open('mix/now.png','rb')))
    elif arg.split()[0] == 'reset' and ctx.author.id == 481591703959240706:
        open('mix/status','w').write('DONE')
        await ctx.message.add_reaction('<:wrk:608810652756344851>')
    elif arg.split()[0] in ['kill','stop','clear','end','break','complete','empty','send']: pass
    elif arg.split()[0] in ['slots','imgs','images']:
        await ctx.send(files=[discord.File(fp=open('mix/s1.png','rb')),discord.File(fp=open('mix/s2.png','rb'))])
    else: return await ctx.send(f'```diff\n-] KEY {arg} WAS NOT FOUND [s1, s2, kill, view]```')
